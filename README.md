...
2. **GET c query‑параметрами** (например, для простых вебхуков вроде Bitrix24 без JSON):
   ...
   > 💡 Если сервис запущен через Docker Compose, тот же запрос можно отправить на порт фронтенда — `http://localhost:3080/webhook/employee?...`. Контейнер nginx проксирует путь `/webhook` на backend, поэтому внешние вебхуки могут использовать единственный открытый порт 3080.
