...
- Для запросов используется эндпоинт `/webhook/employee` (поддерживаются POST с JSON и GET с query-параметрами).
...
Если роботам удобнее работать с query-параметрами, вместо JSON можно отправить GET-запрос:

```
https://<ваш-домен>/webhook/employee?key=obed-webhook-secret&action=add&full_name={{FIO}}&status=true&date={{DATE_CREATE}}
```

Главное — передать секрет `key` и корректно указать действие (`action`). Любой из форматов (JSON или query) попадёт в таблицу без допнастроек.
